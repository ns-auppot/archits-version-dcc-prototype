...
import { Database, Search, X, Info, Users, Shield, CheckCircle, AlertTriangle, XCircle, User, Server, Globe, TrendingUp, TrendingDown, Filter, Clock, ScanSearch, ChevronDown, ChevronUp, ChevronRight, Radar, ArrowRight, Plus, Columns } from "lucide-react";
...
import {
  IDENTITY_TYPE_CONFIG, IDENTITY_FILTER_GROUPS, DB_ROLE_CONFIG, AUTH_TYPE_CONFIG,
...
// ── IaaS structured row type ──────────────────────────────────────────────────

export interface IaaSStructuredDataRow {
  id: string;
  name: string;
  nameSubtitle?: string;
  instanceId: string;
...
interface IaaSStructuredStoreConfig {
  title: string;
  subtitle: string;
  nameColumnLabel: string;
  rows: IaaSStructuredDataRow[];
}

export function getIaaSStructuredStoreConfig(storeType: string): IaaSStructuredStoreConfig {
  switch (storeType) {
    case "rds":
      return {
        title: "RDS Instances",
        ...
        rows: [
          ...
          },
          {
            ...
            instanceId: "db-ANL3K0L1M2N3O",
            account: "987654321098",
            org: "Acme Corp",
            sensitiveFields: 127,
            totalFields: 540,
            dataTypes: ["Personal Names", "Email Addresses", "IP Addresses", "Domain Names", "Company Names", "Region Identifiers", "Corporate Tax IDs", "Securities IDs"],
    ...
    case "azure-sql":
      return {
        ...
        subtitle: "Azure SQL managed database instances",
        nameColumnLabel: "Database Name",
        rows: [
          {
            id: "asql1",
            name: "acme-prod-customers",
            nameSubtitle: "Azure SQL · East US",
            instanceId: "acme-prod-sql-customers",
            account: "acme-prod-subscription",
            org: "Acme Corp",
            sensitiveFields: 96,
            totalFields: 380,
            dataTypes: ["Personal Names", "Email Addresses", "Payment Cards", "Postal Addresses", "Telephone Numbers", "Birthdates"],
            uploadSparkData: generateSparkData(500),
            downloadSparkData: generateSparkData(505),
          },
          {
            id: "asql2",
            name: "acme-prod-hr",
            nameSubtitle: "Azure SQL · East US",
            instanceId: "acme-prod-sql-hr",
            account: "acme-prod-subscription",
            org: "Acme Corp",
            sensitiveFields: 142,
            totalFields: 490,
            ...
            uploadSparkData: generateSparkData(510),
            downloadSparkData: generateSparkData(515),
          },
          {
            id: "asql3",
            name: "acme-analytics-dw",
            nameSubtitle: "Azure SQL · West Europe",
            instanceId: "acme-emea-sql-dw",
            account: "acme-emea-subscription",
            org: "Acme EMEA",
            sensitiveFields: 78,
            totalFields: 620,
            dataTypes: ["Personal Names", "Email Addresses", "IP Addresses", "Company Names", "Region Identifiers", "Corporate Tax IDs"],
            uploadSparkData: generateSparkData(520),
            downloadSparkData: generateSparkData(525),
          },
        ],
      };
    default:
...
interface IaaSDBMetadata {
  ...
  cloudPrincipals: { principal: string; principalType: "user" | "role" | "service"; permissions: string[]; lastAccess: string; stale: boolean; sensitiveAccess: boolean }[];
  dbUsers: { username: string; authType: "native" | "iam" | "aad"; role: "admin" | "readwrite" | "readonly"; lastLogin: string; stale: boolean; sensitiveAccess: boolean }[];
  staleDataSize: string;
  shadowLastAccess: string;
  shadowLastAccessedBy: string;
}

const IAAS_DB_METADATA: Record<string, IaaSDBMetadata> = {
  rds1: {
    provider: "AWS",
    ...
    storageType: "gp3",
    multiAZ: true,
    multiAZLabel: "Multi-AZ standby",
    encryption: "AES-256 (AWS KMS)",
    sslEnforced: true,
    cloudAuth: true,
    cloudAuthLabel: "IAM DB Authentication",
    backupRetention: "7 days (automated)",
    network: "vpc-0a1b2c3d4e5f",
    securityGroups: ["sg-db-prod-users (inbound 5432)", "sg-bastion-allow"],
    publicAccess: "Enabled",
    backupEnabled: true,
    dlpPolicies: ["SSN Detection", "PCI-DSS Card Detection", "PII Comprehensive Scan", "Credential Scanning"],
    complianceFrameworks: ["PCI-DSS", "SOC 2 Type II", "GDPR", "CCPA"],
    lastScanDate: "2026-03-02",
    scanStatus: "complete",
    riskLevel: "critical",
    cloudPrincipals: [
      { principal: "app-prod-role",         principalType: "role",    permissions: ["rds:Connect", "rds-db:connect/app_user"],        lastAccess: "2026-03-05", stale: false, sensitiveAccess: true },
      { principal: "etl-pipeline-role",     principalType: "role",    permissions: ["rds:Connect", "rds-db:connect/etl_user"],         lastAccess: "2026-03-05", stale: false, sensitiveAccess: true },
      { principal: "alice.chen@acme.com",   principalType: "user",    permissions: ["rds:*", "rds-db:connect/*"],                     lastAccess: "2026-03-02", stale: false, sensitiveAccess: true },
      { principal: "dba-admin-role",        principalType: "role",    permissions: ["rds:*"],                                          lastAccess: "2026-03-01", stale: true,  sensitiveAccess: true },
      { principal: "lambda-data-svc",       principalType: "service", permissions: ["rds:Connect", "rds-db:connect/lambda_user"],      lastAccess: "2026-03-04", stale: false, sensitiveAccess: false },
    ],
    dbUsers: [
      { username: "app_user",       authType: "iam",    role: "readwrite", lastLogin: "2026-03-05", stale: false, sensitiveAccess: true },
      { username: "etl_user",       authType: "iam",    role: "readwrite", lastLogin: "2026-03-05", stale: false, sensitiveAccess: true },
      { username: "lambda_user",    authType: "iam",    role: "readonly",  lastLogin: "2026-03-04", stale: false, sensitiveAccess: false },
      { username: "reporting_ro",   authType: "native", role: "readonly",  lastLogin: "2026-03-03", stale: false, sensitiveAccess: false },
      { username: "postgres",       authType: "native", role: "admin",     lastLogin: "2026-03-02", stale: true,  sensitiveAccess: true },
    ],
    staleDataSize: "145 GB",
    shadowLastAccess: "2026-03-05 22:14 UTC",
    shadowLastAccessedBy: "app_user",
  },
  rds2: {
    provider: "AWS",
    engine: "MySQL 8.0",
    ...
    backupRetention: "7 days (automated)",
    network: "vpc-0a1b2c3d4e5f",
    securityGroups: ["sg-db-prod-orders (inbound 3306)", "sg-bastion-allow"],
    publicAccess: "Enabled",
    backupEnabled: true,
    dlpPolicies: ["PCI-DSS Card Detection", "Financial ID Protection", "PII Scan"],
    complianceFrameworks: ["PCI-DSS", "SOC 2 Type II"],
    lastScanDate: "2026-03-01",
    scanStatus: "complete",
    riskLevel: "high",
    cloudPrincipals: [
      { principal: "orders-api-role",       principalType: "role",    permissions: ["rds:Connect", "rds-db:connect/orders_svc"],        lastAccess: "2026-03-05", stale: false, sensitiveAccess: true },
      { principal: "payment-processor-svc", principalType: "service", permissions: ["rds:Connect", "rds-db:connect/payment_user"],      lastAccess: "2026-03-05", stale: false, sensitiveAccess: true },
      { principal: "dba-admin-role",        principalType: "role",    permissions: ["rds:*"],                                           lastAccess: "2026-02-28", stale: true,  sensitiveAccess: true },
      { principal: "analytics-reader-role", principalType: "role",    permissions: ["rds:Connect", "rds-db:connect/readonly_user"],     lastAccess: "2026-03-03", stale: false, sensitiveAccess: false },
    ],
    dbUsers: [
      { username: "orders_svc",    authType: "iam",    role: "readwrite", lastLogin: "2026-03-05", stale: false, sensitiveAccess: true },
      { username: "payment_user",  authType: "iam",    role: "readwrite", lastLogin: "2026-03-05", stale: false, sensitiveAccess: true },
      { username: "readonly_user", authType: "iam",    role: "readonly",  lastLogin: "2026-03-03", stale: false, sensitiveAccess: false },
      { username: "root",          authType: "native", role: "admin",     lastLogin: "2026-02-28", stale: true,  sensitiveAccess: true },
    ],
    staleDataSize: "28 GB",
    shadowLastAccess: "2026-03-05 18:07 UTC",
    shadowLastAccessedBy: "orders_svc",
  },
  rds3: {
    provider: "AWS",
    engine: "PostgreSQL 15.4",
    ...
    storageSize: "2 TB",
    storageType: "io1 (10,000 IOPS)",
    multiAZ: false,
    multiAZLabel: "Single-AZ",
    encryption: "AES-256 (AWS KMS)",
    sslEnforced: true,
    cloudAuth: false,
    cloudAuthLabel: "IAM DB Auth disabled",
    backupRetention: "14 days (automated)",
    network: "vpc-0e1f2a3b4c5d",
    securityGroups: ["sg-analytics-warehouse (inbound 5432)", "sg-data-team-allow"],
    publicAccess: "Disabled",
    backupEnabled: true,
    dlpPolicies: ["IP Address Detection", "Email Address Detection", "Corporate Tax ID Scan"],
    complianceFrameworks: ["SOC 2 Type II", "GDPR"],
    lastScanDate: "2026-03-01",
    scanStatus: "complete",
    riskLevel: "medium",
    cloudPrincipals: [
      { principal: "analytics-etl-role",    principalType: "role",    permissions: ["rds:Connect"],                                     lastAccess: "2026-03-05", stale: false, sensitiveAccess: false },
      { principal: "spark-cluster-svc",     principalType: "service", permissions: ["rds:Connect"],                                     lastAccess: "2026-03-05", stale: false, sensitiveAccess: false },
      { principal: "data-science-role",     principalType: "role",    permissions: ["rds:Connect"],                                     lastAccess: "2026-03-04", stale: false, sensitiveAccess: false },
      { principal: "james.okafor@acme.com", principalType: "user",    permissions: ["rds:*"],                                           lastAccess: "2026-02-27", stale: true,  sensitiveAccess: true },
    ],
    dbUsers: [
      { username: "etl_writer",   authType: "native", role: "readwrite", lastLogin: "2026-03-05", stale: false, sensitiveAccess: true },
      { username: "analytics_ro", authType: "native", role: "readonly",  lastLogin: "2026-03-04", stale: false, sensitiveAccess: false },
      { username: "spark_user",   authType: "native", role: "readwrite", lastLogin: "2026-03-05", stale: false, sensitiveAccess: true },
      { username: "postgres",     authType: "native", role: "admin",     lastLogin: "2026-02-27", stale: true,  sensitiveAccess: true },
    ],
    staleDataSize: "380 GB",
    shadowLastAccess: "2026-03-05 07:22 UTC",
    shadowLastAccessedBy: "etl_writer",
  },
  rds4: {
    provider: "AWS",
    engine: "PostgreSQL 15.4",
    ...
    storageType: "gp3",
    multiAZ: false,
    multiAZLabel: "Single-AZ (staging)",
    encryption: "AES-256 (AWS KMS)",
    sslEnforced: false,
    cloudAuth: false,
    cloudAuthLabel: "IAM DB Auth disabled",
    backupRetention: "3 days (automated)",
    network: "vpc-0a1b2c3d4e5f",
    securityGroups: ["sg-db-staging (inbound 5432)", "sg-dev-team-allow"],
    publicAccess: "Disabled",
    backupEnabled: false,
    dlpPolicies: ["SSN Detection", "PHI Detection", "PII Scan"],
    complianceFrameworks: [],
    lastScanDate: "2026-02-26",
    scanStatus: "partial",
    riskLevel: "high",
    cloudPrincipals: [
      { principal: "hr-staging-role",     principalType: "role",    permissions: ["rds:Connect"],  lastAccess: "2026-03-04", stale: false, sensitiveAccess: false },
      { principal: "dev-admin-role",      principalType: "role",    permissions: ["rds:*"],        lastAccess: "2026-03-03", stale: false, sensitiveAccess: true },
      { principal: "qa-readonly-role",    principalType: "role",    permissions: ["rds:Connect"],  lastAccess: "2026-03-02", stale: false, sensitiveAccess: false },
    ],
    dbUsers: [
      { username: "hr_app",    authType: "native", role: "readwrite", lastLogin: "2026-03-04", stale: false, sensitiveAccess: true },
      { username: "qa_ro",     authType: "native", role: "readonly",  lastLogin: "2026-03-02", stale: false, sensitiveAccess: false },
      { username: "postgres",  authType: "native", role: "admin",     lastLogin: "2026-03-03", stale: true,  sensitiveAccess: true },
    ],
    staleDataSize: "12 GB",
    shadowLastAccess: "2026-03-04 19:33 UTC",
    shadowLastAccessedBy: "hr_app",
  },
  asql1: {
    provider: "Azure",
    engine: "Azure SQL Database",
    region: "East US",
    instanceClass: "Business Critical",
    storageSize: "100 GB",
    storageType: "Premium SSD",
    multiAZ: true,
    multiAZLabel: "Zone-redundant (3 replicas)",
    encryption: "Transparent Data Encryption (AES-256)",
    sslEnforced: true,
    cloudAuth: true,
    cloudAuthLabel: "Azure AD (Entra ID) Authentication",
    backupRetention: "35 days (geo-redundant backup)",
    network: "vnet-acme-prod-eastus",
    securityGroups: ["nsg-sql-prod (inbound 1433)", "firewall: 10.0.0.0/16 allowed"],
    publicAccess: "Enabled",
    backupEnabled: true,
    dlpPolicies: ["PCI-DSS Card Detection", "PII Comprehensive Scan", "Email Address Detection"],
    complianceFrameworks: ["PCI-DSS", "GDPR", "SOC 2 Type II"],
    lastScanDate: "2026-03-02",
    scanStatus: "complete",
    riskLevel: "critical",
    cloudPrincipals: [
      { principal: "app-prod-managed-identity",    principalType: "service", permissions: ["db_datareader", "db_datawriter"],      lastAccess: "2026-03-05", stale: false, sensitiveAccess: true },
      { principal: "etl-pipeline-sp",              principalType: "service", permissions: ["db_datareader", "db_datawriter"],      lastAccess: "2026-03-05", stale: false, sensitiveAccess: true },
      { principal: "dba-admin-group",              principalType: "role",    permissions: ["db_owner"],                            lastAccess: "2026-03-02", stale: false, sensitiveAccess: true },
      { principal: "priya.sharma@acme.com",        principalType: "user",    permissions: ["db_datareader"],                       lastAccess: "2026-03-01", stale: true,  sensitiveAccess: false },
    ],
    dbUsers: [
      { username: "app_prod_mi",   authType: "aad",    role: "readwrite", lastLogin: "2026-03-05", stale: false, sensitiveAccess: true },
      { username: "etl_svc_sp",    authType: "aad",    role: "readwrite", lastLogin: "2026-03-05", stale: false, sensitiveAccess: true },
      { username: "reporting_ro",  authType: "native", role: "readonly",  lastLogin: "2026-03-03", stale: false, sensitiveAccess: false },
      { username: "sqladmin",      authType: "native", role: "admin",     lastLogin: "2026-03-01", stale: true,  sensitiveAccess: true },
    ],
    staleDataSize: "18 GB",
    shadowLastAccess: "2026-03-05 20:45 UTC",
    shadowLastAccessedBy: "app_prod_mi",
  },
  asql2: {
    provider: "Azure",
    engine: "Azure SQL Database",
    ...
    lastScanDate: "2026-03-03",
    scanStatus: "complete",
    riskLevel: "critical",
    cloudPrincipals: [
      { principal: "hr-app-managed-identity",  principalType: "service", permissions: ["db_datareader", "db_datawriter"],  lastAccess: "2026-03-05", stale: false, sensitiveAccess: true },
      { principal: "hr-etl-sp",                principalType: "service", permissions: ["db_datawriter"],                   lastAccess: "2026-03-04", stale: false, sensitiveAccess: true },
      { principal: "hr-admin-group",           principalType: "role",    permissions: ["db_owner"],                        lastAccess: "2026-03-02", stale: false, sensitiveAccess: true },
      { principal: "hipaa-auditor-group",      principalType: "role",    permissions: ["db_datareader"],                   lastAccess: "2026-02-28", stale: true,  sensitiveAccess: false },
    ],
    dbUsers: [
      { username: "hr_app_mi",      authType: "aad",    role: "readwrite", lastLogin: "2026-03-05", stale: false, sensitiveAccess: true },
      { username: "hr_etl_sp",      authType: "aad",    role: "readwrite", lastLogin: "2026-03-04", stale: false, sensitiveAccess: true },
      { username: "hipaa_audit_ro", authType: "native", role: "readonly",  lastLogin: "2026-02-28", stale: true,  sensitiveAccess: false },
      { username: "sqladmin",       authType: "native", role: "admin",     lastLogin: "2026-03-02", stale: false, sensitiveAccess: true },
    ],
    staleDataSize: "45 GB",
    shadowLastAccess: "2026-03-05 14:22 UTC",
    shadowLastAccessedBy: "hr_app_mi",
  },
  asql3: {
    provider: "Azure",
    engine: "Azure SQL Database",
    region: "West Europe",
    instanceClass: "Standard S3",
    storageSize: "500 GB",
    storageType: "Standard SSD",
    multiAZ: false,
    multiAZLabel: "Locally redundant",
    encryption: "Transparent Data Encryption (AES-256)",
    sslEnforced: true,
    cloudAuth: true,
    cloudAuthLabel: "Azure AD (Entra ID) Authentication",
    backupRetention: "14 days (locally redundant backup)",
    network: "vnet-acme-emea-westeu",
    securityGroups: ["nsg-sql-analytics (inbound 1433)", "firewall: 10.1.0.0/16 allowed"],
    publicAccess: "Enabled",
    backupEnabled: true,
    dlpPolicies: ["Email Address Detection", "IP Address Detection", "Corporate Tax ID Scan"],
    complianceFrameworks: ["GDPR", "ISO 27001"],
    lastScanDate: "2026-03-01",
    scanStatus: "complete",
    riskLevel: "medium",
    cloudPrincipals: [
      { principal: "analytics-dw-mi",          principalType: "service", permissions: ["db_datareader", "db_datawriter"], lastAccess: "2026-03-05", stale: false, sensitiveAccess: true },
      { principal: "power-bi-sp",              principalType: "service", permissions: ["db_datareader"],                  lastAccess: "2026-03-04", stale: false, sensitiveAccess: false },
      { principal: "data-team-emea-group",     principalType: "role",    permissions: ["db_datareader"],                  lastAccess: "2026-03-03", stale: false, sensitiveAccess: false },
      { principal: "sofia.novak@acme-emea.com",principalType: "user",    permissions: ["db_owner"],                       lastAccess: "2026-02-25", stale: true,  sensitiveAccess: true },
    ],
    dbUsers: [
      { username: "dw_mi",          authType: "aad",    role: "readwrite", lastLogin: "2026-03-05", stale: false, sensitiveAccess: true },
      { username: "powerbi_sp",     authType: "aad",    role: "readonly",  lastLogin: "2026-03-04", stale: false, sensitiveAccess: false },
      { username: "analytics_team", authType: "native", role: "readonly",  lastLogin: "2026-03-03", stale: false, sensitiveAccess: false },
      { username: "sqladmin",       authType: "native", role: "admin",     lastLogin: "2026-02-25", stale: true,  sensitiveAccess: true },
    ],
    staleDataSize: "62 GB",
    shadowLastAccess: "2026-03-05 10:11 UTC",
    shadowLastAccessedBy: "dw_mi",
  },
};

function getInstanceMeta(rowId: string): IaaSDBMetadata {
  return IAAS_DB_METADATA[rowId] ?? IAAS_DB_METADATA["rds1"];
}

function getTotalIdentitiesCount(rowId: string): number {
  const meta = getInstanceMeta(rowId);
  return meta.cloudPrincipals.length + meta.dbUsers.length;
}
...
function OverviewTabContent({ row, onNavigateToAccess, onTabChange }: { row: IaaSStructuredDataRow; onNavigateToAccess: (f: AccessFilter) => void; onTabChange?: (tab: PanelTab) => void }) {
  const meta = getInstanceMeta(row.id);
  const allUsers = [...meta.cloudPrincipals.filter(p => p.principalType !== "role"), ...meta.dbUsers];
  const staleCount = allUsers.filter(u => u.stale).length;
  const sensitiveCount = allUsers.filter(u => u.sensitiveAccess).length;
  ...
  return (
    <div className="px-5 py-4 space-y-4">
      ...
      <div className="grid grid-cols-12 gap-4">
        <div className="col-span-8 bg-white dark:bg-slate-900 border border-border rounded-lg px-3.5 py-3">
          ...
          <div className="flex gap-0">
            ...
            {/* Right: DB structure counts */}
            {(() => {
              const h = row.id.split("").reduce((a, c, i) => a + c.charCodeAt(0) * (i + 1), 0);
              const dbCount     = 1 + (h % 4);
              const schemaCount = 3 + (h % 12);
              const tableCount  = Math.max(4, Math.round(row.totalFields / (8 + (h % 5))));
              const items = [
                { label: "Databases", count: dbCount },
                { label: "Schemas",   count: schemaCount },
                { label: "Tables",    count: tableCount },
              ];
              const scanH = (row.id.charCodeAt(0) + row.id.length) % 24;
              const scanM = ((row.id.charCodeAt(1) ?? 0) + row.id.length * 3) % 60;
              const scanD = ["Apr 8, 2026", "Apr 9, 2026", "Apr 9, 2026", "Apr 10, 2026"][scanH % 4];
              const scanHH = scanH % 12 === 0 ? 12 : scanH % 12;
              const scanMM = String(scanM).padStart(2, "0");
              const scanAmpm = scanH < 12 ? "AM" : "PM";
              const scanLabel = `${scanD}, ${scanHH}:${scanMM} ${scanAmpm}`;
              return (
                <div className="flex-1 min-w-0 space-y-2 pl-3">
                  ...
                  {items.map((item) => (
                    <div key={item.label} className="flex items-center justify-between gap-2">
                      ...
                      <span className="text-text-bright" style={{ fontSize: "11px", fontWeight: 600 }}>{item.count.toLocaleString()}</span>
                    </div>
                  ))}
                  <InfoRow label="Last Discovery Scan" value={scanLabel} />
                </div>
              );
            })()}
          </div>
        ...
        <div className="col-span-4 bg-white dark:bg-slate-900 border border-border rounded-lg px-3.5 py-3 space-y-2">
          <div className="text-muted-foreground mb-2" style={{ fontSize: "10px", fontWeight: 600, letterSpacing: "0.04em" }}>Security Configuration</div>
            <InfoRow label="Encryption at Rest" value={<SecurityStatus enabled={meta.encryption.toLowerCase() !== "none"} />} />
          ...
          <div className="flex items-stretch px-3.5 py-3 gap-0">
            ...
            <div className="flex-1 min-w-0 flex flex-col justify-center gap-2">
              ...
              {(() => {
                ...
                for (const p of meta.cloudPrincipals) {
                  if (p.principalType === "role" || !p.sensitiveAccess) continue;
                  let k: string;
                  if (p.stale) k = "unknown-identity";
                  else if (p.principal.includes("@acme.com")) k = "internal-user";
                  else if (p.principal.includes("@")) k = "external-user";
                ...
                for (const u of meta.dbUsers) {
                  ...
                  else if (u.authType === "native") k = "unmapped-local-user";
      ...
      {/* ── Activity ── */}
      <ActivityOverviewSection row={row} gradId={`iaasStrOvGrad-${row.id}`} />
...
// (IDENTITY_TYPE_CONFIG, IDENTITY_FILTER_GROUPS, DB_ROLE_CONFIG, AUTH_TYPE_CONFIG,
//  ROLE_MEMBER_POOL, getRoleMembers, classifyCloudPrincipal, classifyRoleMember,
//  UnifiedIdentity, AccessFilter → identity-shared.tsx)

function classifyIaaSDbUser(username: string, authType: "iam" | "aad" | "native", stale: boolean): IdentityTypeName {
  if (stale) return "unknown-identity";
  if (authType === "native") return "unmapped-local-user";
  const n = username.toLowerCase();
  if (n.endsWith("_mi") || n.endsWith("_sp") || n.includes("_svc") || n.includes("pipeline") || n.includes("service")) return "service-account";
...
// Build a flat list of unified identities: roles are expanded to their members
function buildUnifiedIdentities(meta: IaaSDBMetadata): UnifiedIdentity[] {
  const map = new Map<string, UnifiedIdentity>();

  // 1. Direct cloud identities (user / service — not role)
  for (const p of meta.cloudPrincipals) {
    if (p.principalType === "role") continue;
    const resolved = EMAIL_TO_NAME[p.principal];
    const displayName = resolved ? resolved.name : p.principal;
    const email = resolved ? resolved.email : (p.principal.includes("@") ? p.principal : undefined);
    map.set(displayName, {
      key: `cloud-${p.principal}`,
      name: displayName,
      email,
      identityType: classifyCloudPrincipal(p.principal, p.principalType, p.stale),
      assignedRoles: [],
      permissions: [...p.permissions],
      lastAccess: p.lastAccess,
      stale: p.stale,
      sensitiveAccess: p.sensitiveAccess,
      source: "cloud",
    });
  }

  // 2. Expand role entries → members (roles become assigned-role attributes)
  for (const p of meta.cloudPrincipals) {
    if (p.principalType !== "role") continue;
    for (const m of getRoleMembers(p.principal)) {
      if (map.has(m.name)) {
        const existing = map.get(m.name)!;
        if (!existing.assignedRoles.includes(p.principal)) existing.assignedRoles.push(p.principal);
        for (const perm of p.permissions) {
          if (!existing.permissions.includes(perm)) existing.permissions.push(perm);
        }
        if (p.sensitiveAccess) existing.sensitiveAccess = true;
      } else {
        map.set(m.name, {
          key: `role-${m.name}`,
          name: m.name,
          email: (m as any).email,
          identityType: classifyRoleMember(m.identityKey, p.stale),
          assignedRoles: [p.principal],
          permissions: [...p.permissions],
          lastAccess: p.lastAccess,
          stale: p.stale,
          sensitiveAccess: p.sensitiveAccess,
  ...
  // 3. Database users (keyed with "db:" prefix to avoid name collisions)
  for (const u of meta.dbUsers) {
    map.set(`db:${u.username}`, {
      key: `db-${u.username}`,
      name: u.username,
      identityType: classifyIaaSDbUser(u.username, u.authType, u.stale),
      assignedRoles: [],
      permissions: [],
...
function AccessTabContent({ row, accessFilter, onClearFilter, onSelectIdentity }: { row: IaaSStructuredDataRow; accessFilter: AccessFilter; onClearFilter: () => void; onSelectIdentity?: (identity: UnifiedIdentity | null) => void }) {
  const allIdentities = useMemo(() => buildUnifiedIdentities(getInstanceMeta(row.id)), [row.id]);
  const meta = getInstanceMeta(row.id);
  const filters = useIdentityFilters(allIdentities, accessFilter);
  const cloudCount = allIdentities.filter(i => i.source === "cloud").length;
  const dbCount    = allIdentities.filter(i => i.source === "db").length;
...
function computeFieldDataTypeBreakdown(row: IaaSStructuredDataRow) {
...
function StatWithTooltip({ value, label, tooltip }: { value: string; label: string; tooltip: string }) {
  return (
    <span className="relative group inline-flex items-center gap-0.5 cursor-default select-none">
      ...
      <span
        className="absolute bottom-full left-0 mb-1.5 w-52 px-2.5 py-2 bg-[#0f1a2e] border border-border rounded-md shadow-xl text-white opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-50 whitespace-normal"
...
function DataTypesTabContent({ row }: { row: IaaSStructuredDataRow }) {
  const breakdown = useMemo(() => computeFieldDataTypeBreakdown(row), [row]);
                  ...
                  {isExpanded && (
                    <div className="mt-3 pt-3 border-t border-border/50 space-y-2.5">
                      ...
                      <div className="flex items-center gap-4">
                        <div>
                          <div className="text-muted-foreground" style={{ fontSize: "10px", fontWeight: 500 }}>Column Coverage</div>
...
function SensitiveColumnsTabContent({
          ...
          {filteredFields.length > 0 ? (
            ...
            <div className="text-center text-muted-foreground mt-6 py-4" style={{ fontSize: "12px" }}>
              {hasActiveDataTypeFilter && !search
                ? `No columns found for the selected data type${dataTypeFilter.length > 1 ? "s" : ""}`
...
export function IaaSStructuredRowPanelContent({ row, storeType = "rds" }: { row: IaaSStructuredDataRow; storeType?: string }) {
  ...
  const handleSelectIdentity = useCallback((id: { username: string; role: string; identityType: string }) => {
    const identity: UnifiedIdentity = {
      ...
      assignedRoles: [id.role], permissions: [id.role],
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {selectedIdentity && (
        <SidePanel open onClose={() => { setSelectedIdentity(null); setIdentityFromFieldIdx(null); }}
          onBack={() => { setSelectedIdentity(null); if (identityFromFieldIdx !== null) { setFullDetailIdx(identityFromFieldIdx); setIdentityFromFieldIdx(null); } }}
      ...
      {fullDetailIdx !== null && (() => {
        ...
        return (
          <SidePanel open onClose={closeFullDetail} onBack={closeFullDetail}
            title={colName} subtitle={`${tableName} · ${row.name}`}
            width="min(840px, 90vw)" zIndex={70} hideBackdrop hideHeader stacked
          >
            <FieldDetailPane field={field} onBack={closeFullDetail} onClose={closeFullDetail} onSelectIdentity={handleSelectIdentity} />
          </SidePanel>
        );
      })()}
    </div>
...
function TabbedPanelContent({
  ...
  return (
    <div className="flex flex-row h-full">
      <div className="shrink-0 flex flex-col gap-0 py-2 border-r border-border w-[196px]">
        {PANEL_TABS.map((tab) => {
      ...
      <div className="flex-1 overflow-y-auto min-h-0">
        {activeTab === "overview"        ? <OverviewTabContent row={row} onNavigateToAccess={handleNavigateToAccess} onTabChange={onTabChange} />
         ...
         : <SensitiveColumnsTabContent row={row} onViewFullDetails={onViewFullDetails} />}
      </div>
    </div>
  );
}
...
function compareRows(a: IaaSStructuredDataRow, b: IaaSStructuredDataRow, key: string): number {
  switch (key) {
...
export function StructuredDataStoreTableIaaS({ storeType }: { storeType: string }) {
  const config = getIaaSStructuredStoreConfig(storeType);
  const [panelRow, setPanelRow] = useState<IaaSStructuredDataRow | null>(null);
  const [activeTab, setActiveTab] = useState<PanelTab>("overview");
  ...
  const openPanel = useCallback((row: IaaSStructuredDataRow, tab: PanelTab = "overview") => {
  ...
  const handleCellClick = useCallback((e: React.MouseEvent, row: IaaSStructuredDataRow) => { e.stopPropagation(); openPanel(row, "sensitive-fields"); }, [openPanel]);
  const handleDataTypeCellClick = useCallback((e: React.MouseEvent, row: IaaSStructuredDataRow) => { e.stopPropagation(); openPanel(row, "sensitive-fields"); }, [openPanel]);
  const handleIdentitiesClick = useCallback((e: React.MouseEvent, row: IaaSStructuredDataRow) => { e.stopPropagation(); openPanel(row, "access"); }, [openPanel]);
        ...
        >
          <IdentityDetailPanel
            ...
            row={{
              ...
              Domain: selectedIdentity.name.includes("@") ? selectedIdentity.name.split("@")[1] : "acme.com",
