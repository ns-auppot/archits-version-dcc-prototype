...
export function EntityPopover({ entity, cursorPos }: EntityPopoverProps) {
  const viewportHeight = typeof window !== 'undefined' ? window.innerHeight : 800
  const viewportWidth  = typeof window !== 'undefined' ? window.innerWidth  : 1200

  let left = cursorPos.x + OFFSET_X
  let top  = cursorPos.y + OFFSET_Y

  // Clamp horizontally
  left = Math.min(left, viewportWidth - POPOVER_WIDTH - 8)
  left = Math.max(8, left)

  // Flip above cursor if too close to bottom
  const ESTIMATED_HEIGHT = 240
  if (top + ESTIMATED_HEIGHT > viewportHeight - 8) {
    top = cursorPos.y - OFFSET_Y - ESTIMATED_HEIGHT
  }
  ...
  const hasContent = contextFields.length > 0 || sortedBadges.length > 0
