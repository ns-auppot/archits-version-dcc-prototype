...
import { FieldDetailPane } from "./ForensicDetailPane";
...
export function StructuredRowPanelContent({ row }: { row: StructuredDataRow }) {
  return (
    <div className="px-5 py-4">
      ...
      <div className="mb-5">
        <div
          className="text-muted-foreground uppercase tracking-wide mb-3"
          style={{ fontSize: "10px", fontWeight: 600, letterSpacing: "0.05em" }}
        >
          Activity (30 days)
        </div>
        <div className="space-y-3">
          <div>
            ...
            <div className="h-8">
              <Sparkline data={row.uploadSparkData} color="var(--color-primary)" />
            </div>
          </div>
          <div>
            <div className="text-muted-foreground mb-1" style={{ fontSize: "11px" }}>
            ...
            <div className="h-8">
              <Sparkline data={row.downloadSparkData} color="#06b6d4" />
            </div>
          </div>
        </div>
      </div>
...
// ── Cell Panel: Sensitive Fields List ─────────────────────────────────────────
    ...
    <div className="flex h-full">
        ...
        <div className="px-3 pt-3 pb-2 border-b border-border shrink-0">
          ...
          <div className="flex items-center gap-2 mt-2">
            ...
            <span className="text-muted-foreground" style={{ fontSize: "11px" }}>
              ...
              {row.sensitiveFields === 1 ? "field" : "fields"}
            </span>
          </div>
        </div>
      ...
      {selectedField && (
        <div className="flex-1 overflow-y-auto min-w-0">
          <FieldDetailPane field={selectedField} />
        </div>
      )}
    </div>
  );
...
const SORT_COLUMNS: SortColumnDef[] = [
  { key: "name", label: "Name" },
  { key: "sensitiveFields", label: "# Sensitive Fields" },
...
export function StructuredDataStoreTable({ storeType }: { storeType: string }) {
  ...
  return (
    <div className="flex-1 overflow-auto">
      ...
      {/* ── Per-cell side panel (sensitive fields) ── */}
      <SidePanel
        open={panel?.type === "cell"}
        onClose={closePanel}
        title={panel?.type === "cell" ? "Sensitive Fields" : ""}
        subtitle={panel?.type === "cell" ? panel.row.name : undefined}
        width={cellSelectedIdx !== null ? 920 : 420}
      >
        {panel?.type === "cell" && (
          <CellPanelContent
            row={panel.row}
